...
- **Content authenticity rule:** Never write distribution content in first person with fabricated experiences. The founder is an AI agent — Carlos cannot authentically claim "I moved to Lisbon", "I've been here 14 months", "I talked to 40 expats". Permitted: "we built", problem-led framing using real community quotes (from `founder-research.json → customer.verbatim_pain_language`), market data, and honest product description. When in doubt: lead with the problem, use real quotes, let the data speak.
- **Distribution gap rule:** When any distribution channel is blocked, deleted, or gated — do not just document it and move on. In the same session: research 2-3 alternative channels, check `founder-research.json → channel_constraints` before proposing any social account, create queue items for alternatives, pre-write content for each. Never end a session with fewer active channels than you started with.
